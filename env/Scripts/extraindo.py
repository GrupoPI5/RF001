...
def download_b3_quotes(symbols, start_date, end_date):
    ...
    for symbol in symbols:
        ...
        data = data.rename(columns={'open': 'abertura', 'high': 'alta', 'low': 'baixa',
...
concatenated_quotes = pd.concat(historical_quotes.values(), keys=historical_quotes.keys())

# Resetando o índice
concatenated_quotes.reset_index(level=0, inplace=True)
concatenated_quotes.rename(columns={'level_0': 'symbol'}, inplace=True)
